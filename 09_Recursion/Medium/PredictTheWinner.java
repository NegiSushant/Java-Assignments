...
Ques_486------>>>>>>
                    ...
                    with a score of 0. At each turn, the player takes one of the numbers from either end of the
                    array (i.e., nums[0] or nums[nums.length - 1]) which reduces the size of the array by 1.
                    The player adds the chosen number to their score. The game ends when there are no more elements
                    in the array.
...
public class PredictTheWinner {
    ...
    public static int winner(int[] nums, int start, int end, int turn){
        if(start == end){
            return turn * nums[start];
        }
        int player_1 = turn * nums[start] + winner(nums, start + 1, end, -turn);
        int player_2 = turn * nums[end] + winner(nums, start, end - 1, -turn);
        return turn * Math.max(turn * player_1, turn * player_2);
    }
}
/*
return winner(nums, 0, nums.length - 1, 1) >= 0;
